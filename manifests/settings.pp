...
class barman::settings (
  ...
  Integer $archiver_batch_size           = undef,
  Pattern['^(rsync|postgres)$'] $backup_method                 = undef,
  Pattern['^exclusive_backup$', '^concurrent_backup$'] $backup_options = 'exclusive_backup',
  Integer $bandwidth_limit               = undef,
  Pattern['^[0-9]+$','^false$'] $basebackup_retry_sleep = false,
  Pattern['^[0-9]+$','^false$'] $basebackup_retry_times = false,
  Integer $check_timeout                 = undef,
  String $custom_compression_filter     = undef,
  String $custom_decompression_filter   = undef,
  ...
  String $log_level                     = undef,
  Pattern['^[0-9]+$'] $minimum_redundancy = '0',
  Boolean $network_compression           = undef,
  Integer $parallel_jobs                 = undef,
  Stdlib::Absolutepath $path_prefix                   = undef,
  ...
  Pattern['^get-wal$'] $recovery_options = undef,
  ...
  String $slot_name                                                                                            = undef,
  Boolean $streaming_archiver                                                                                  = false,
  Integer $streaming_archiver_batch_size                                                                    = undef,
  String $streaming_archiver_name       = undef,
  String $streaming_backup_name         = undef,
  String $tablespace_bandwidth_limit    = undef,
